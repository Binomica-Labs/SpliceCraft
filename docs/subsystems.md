...
**API download (sweep #29, 2026-06-21):** fetch a run straight from a user's Plasmidsaurus account by 6-char item code instead of a hand-downloaded zip. The network client lives in **`splicecraft_search`** (`_plasmidsaurus_*`): `_plasmidsaurus_oauth_token` (OAuth2 client-credentials `POST /oauth/token`, HTTP Basic) → `_plasmidsaurus_api_get` (Bearer JSON GET) backing `_plasmidsaurus_list_items` (`GET /api/items?limit=…`) + `_plasmidsaurus_result_link` (`GET /api/item/<code>/results` → `{link}`) → `_plasmidsaurus_download_zip` (dedicated hardened streamer — HTTPS-only hardened opener, content-type guard, `PK\x03\x04` magic check, `_PLASMIDSAURUS_DOWNLOAD_MAX_BYTES = 600 MB` cap, disk pre-check, L2-chokepoint atomic write; kept SEPARATE from the HMM `_stream_download_to_path` so the shipped gzip path carries no risk). `_plasmidsaurus_fetch_item_zip` orchestrates creds→token→link→download into a temp dir. `_sanitize_plasmidsaurus_item_code` enforces `^[A-Z0-9]{6}$` before any URL interpolation. **Import:** `_plasmidsaurus_zip_to_entries(zip, run_id)` (in **`splicecraft_fileio`**) mirrors the bulk-align add-path entry shape (`source: plasmidsaurus:<code>:<sample>`), used by the agent endpoint. **Credentials** resolve env-first (`PLASMIDSAURUS_CLIENT_ID` / `_SECRET`) then settings (`_plasmidsaurus_credentials`); the secret key is in `_SENSITIVE_SETTING_KEYS` (redacted in `_set_setting`'s log/event) and absent from `_AGENT_SETTINGS_ALLOWLIST` (agent can't read/set it). **Order picker (2026-07-27):** `PlasmidsaurusFetchModal` now ENUMERATES the account on open — `_plasmidsaurus_orders(cid, sec, force=)` (hub-side) wraps token+list behind a 120 s process cache keyed on a SHA-256 of the credential pair (so a Settings change misses the cache; the raw values are never stored), because the account is capped at 10 requests/min and reopening the modal would otherwise re-spend it. The pure `_plasmidsaurus_order_rows(items, include_empty=)` builds the display rows — drops codes failing `^[A-Z0-9]{6}$`, hides no-results orders unless `include_empty`, routes dates through `_history_human_dt` (universal slash-free form) and order/product names through `_sanitize_label` (remote input into a terminal), and caps the two name columns so the table doesn't scroll sideways. It is PURE on purpose: the selection logic is unit-tested without driving a headless DataTable. It also **de-duplicates by code**: the code is the DataTable row key and Textual's `add_row` RAISES `DuplicateKey` on a repeat, so one duplicated row in the server's listing would take the whole modal down rather than render a row twice. The download REUSES the listing's bearer token and re-authenticates once on a 401/403 (a cached token can age out between listing and download). The post-listing `tbl.focus()` is SKIPPED while the code box has focus — the listing lands asynchronously, so a user typing a shared code as the modal opened would otherwise have the caret yanked mid-word. **Surfaces:** GUI `PlasmidsaurusFetchModal` (opened by the **Fetch by item code from Plasmidsaurus API** button on the General sub-tab; `@work` download worker → `_load_library` append + `_save_library` → LibraryPanel refresh) + agent endpoints `plasmidsaurus-items` (read) / `download-plasmidsaurus` (write). Settings UI: **Settings → Plasmidsaurus API** (Client ID + password-masked Secret + Save/Clear). Tests: `tests/test_plasmidsaurus_api.py` (network fully mocked at `_build_hardened_url_opener`).

**Live-API quirks (verified against a real account 2026-07-27, `[INV-169]` — none of this is in the vendor docs).** `GET /api/items` **defaults to 100 items and truncates silently**; `?limit=N` is honoured to a **hard max of 1000** (over that is a 400, not a cap) and **there is no pagination at all** — `offset` / `page` / `skip` / `per_page` / `cursor` are accepted and ignored, so past 1000 orders the tail is unreachable. The account is rate-limited to **10 requests per minute** (`429 {"message": "10 per 1 minute"}`), and one GUI fetch already costs 3. Roughly **40% of a real listing is `ups_shipping_label` orders** — marked `status: "complete"` with a null `done_date`, sorted to the TOP, and 404 on `/results`; `status` also takes `"canceled"`. Pick by the derived **`has_results`** (`_plasmidsaurus_item_has_results`: complete AND dated AND not in `_PLASMIDSAURUS_NON_RESULT_PRODUCTS`), never by `status` alone. Items also carry **`order_name`**, the user's own label for the order. Probing live requires the `XDG_DATA_HOME` sandbox **plus** `_authorize_writes_for_sandbox` (the zip streamer hits the L2 chokepoint even though the probe is otherwise read-only).
